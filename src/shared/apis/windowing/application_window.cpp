...
namespace pbr::shared::apis::windowing {
    ...
    void application_window::update() {
        ...
        if (vkAllocateCommandBuffers(this->_device, &command_buffer_allocate_info, &buffer) != VK_SUCCESS) {
            this->_log_manager->log_message("Failed to create command buffers.", apis::logging::log_levels::error);
            return;
        }
        ...
        if (vkBeginCommandBuffer(buffer, &begin_info) != VK_SUCCESS) {
            this->_log_manager->log_message("Failed to begin command buffer: " + std::to_string(0), apis::logging::log_levels::error);
            return;
        }
        ...
        render_pass_info.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
        render_pass_info.renderPass = this->_render_pass;
        render_pass_info.framebuffer = this->_swap_chain_framebuffers[image_index];
        render_pass_info.renderArea.offset = {0, 0};
        render_pass_info.renderArea.extent = this->_swap_chain_extent;
        ...
        if (vkEndCommandBuffer(buffer) != VK_SUCCESS) {
            this->_log_manager->log_message("Failed to end command buffer: " + std::to_string(0), apis::logging::log_levels::error);
            return;
        }
