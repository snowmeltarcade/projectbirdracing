...
namespace pbr::shared::apis::graphics {
    bool vulkan_graphics_manager::load_api() {
        this->_log_manager->log_message("Loading the graphics API...", apis::logging::log_levels::info);
        ...
        if (SDL_Vulkan_LoadLibrary("./libMoltenVK.dylib") < 0) {
            this->_log_manager->log_message("Failed to load MoltenVK library with error:", apis::logging::log_levels::error);
            this->_log_manager->log_message(SDL_GetError(), apis::logging::log_levels::error);
            return false;
        }
#endif

        this->_log_manager->log_message("Loaded the graphics API.", apis::logging::log_levels::info);

        return true;
    }

    bool vulkan_graphics_manager::initialize() {
        this->_log_manager->log_message("Initializing the graphics manager...", apis::logging::log_levels::info);
        ...
        this->_log_manager->log_message("Initialized the graphics manager.", apis::logging::log_levels::info);

        return true;
    }

    bool vulkan_graphics_manager::shutdown() noexcept {
        this->_log_manager->log_message("Shutting down the graphics API...", apis::logging::log_levels::info);

        SDL_VideoQuit();

        this->_log_manager->log_message("Shut down the graphics API.", apis::logging::log_levels::info);
